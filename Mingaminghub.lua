...
-------------------------------------------------
-- CONFIG SAVE / LOAD (nhớ trạng thái khi execute lại)
-------------------------------------------------
local CONFIG_FILE = "MinGamingHub_Config.json"
_G.__ConfigLoading = false
_G.__ConfigDirty = false
    ...
    if Options then
        for name, option in pairs(Options) do
            ...
            if ok and value ~= nil then
                -- Chỉ lưu kiểu đơn giản (bool/number/string)
...
function SaveMinGamingConfig()
    if _G.__ConfigLoading then
        return
    end
...
function ApplyMinGamingConfig(data)
    if not data then
        return
    end

    _G.__ConfigLoading = true

    -- Globals
    if data.Globals then
        if data.Globals.ChooseWeapon ~= nil then ChooseWeapon = data.Globals.ChooseWeapon end
        if data.Globals.SelectWeapon ~= nil then SelectWeapon = data.Globals.SelectWeapon end
        if data.Globals.SelectBoss ~= nil then SelectBoss = data.Globals.SelectBoss end
        if data.Globals.SelectMaterial ~= nil then SelectMaterial = data.Globals.SelectMaterial end
        if data.Globals.SelectChip ~= nil then SelectChip = data.Globals.SelectChip end
        if data.Globals.Fast_Delay ~= nil then _G.Fast_Delay = data.Globals.Fast_Delay end
        if data.Globals.AutoLevel ~= nil then _G.AutoLevel = data.Globals.AutoLevel end
    end

    -- Fluent Options (SetValue sẽ kích hoạt OnChanged → gán _G)
    if data.Options and Options then
...
function ResetMinGamingConfig()
    -- Xóa file config
    if isfile and isfile(CONFIG_FILE) and delfile then
        pcall(function()
            delfile(CONFIG_FILE)
        end)
    ...
    -- Reset globals về mặc định ban đầu
    ChooseWeapon = "Melee"
    SelectWeapon = nil
    SelectBoss = ""
    SelectMaterial = ""
    ...
    -- Tắt toàn bộ toggle Fluent về Default/false nếu có thể
    if Options then
        for name, option in pairs(Options) do
            if option and option.SetValue then
                pcall(function()
                    local t = typeof(option.Value)
                    if t == "boolean" then
    ...
    -- Một số toggle mặc định true trong script gốc
    if Options.ToggleRejoin and Options.ToggleRejoin.SetValue then
        pcall(function() Options.ToggleRejoin:SetValue(true) end)
    end
    if Options.ToggleNoClip and Options.ToggleNoClip.SetValue then
        pcall(function() Options.ToggleNoClip:SetValue(true) end)
    end
    if Options.ToggleWalkonWater and Options.ToggleWalkonWater.SetValue then
        pcall(function() Options.ToggleWalkonWater:SetValue(true) end)
    end
    if Options.ToggleAntiBand and Options.ToggleAntiBand.SetValue then
        pcall(function() Options.ToggleAntiBand:SetValue(true) end)
    end
    if Options.ToggleSpawnCake and Options.ToggleSpawnCake.SetValue then
...
-- Tự động lưu định kỳ khi có thay đổi
task.spawn(function()
    while task.wait(2) do
        if _G.__ConfigDirty and not _G.__ConfigLoading then
            SaveMinGamingConfig()
        end
    end
end)

-- Đánh dấu dirty khi _G thay đổi qua metatable đơn giản không khả thi;
-- dùng hook nhẹ: lưu mỗi 8 giây luôn (an toàn)
task.spawn(function()
    while task.wait(8) do
        if not _G.__ConfigLoading then
            SaveMinGamingConfig()
        end
    end
end)
...
-------------------------------------------------
-- TAB SETTING - Cài Đặt (Reset + Config)
-------------------------------------------------
Tabs.Setting:AddButton({
    Title = "Reset Tất Cả Về Mặc Định",
    Description = "Xóa config đã lưu và tắt/bật lại như lúc execute lần đầu",
    Callback = function()
        ResetMinGamingConfig()
        Fluent:Notify({
            Title = "Min Gaming",
...
Tabs.Setting:AddButton({
    Title = "Lưu Cấu Hình Ngay",
    Description = "Ghi nhớ toàn bộ toggle/dropdown hiện tại",
    Callback = function()
        SaveMinGamingConfig()
        Fluent:Notify({
            Title = "Min Gaming",
...
Tabs.Setting:AddParagraph({
    Title = "Config",
    Content = "Script tự nhớ toggle khi execute lại (file: MinGamingHub_Config.json)"
})
...
-- Load config đã lưu (bật lại toggle như lần trước)
task.spawn(function()
    task.wait(0.35)
    local saved = LoadMinGamingConfig()
    if saved then
        ApplyMinGamingConfig(saved)
        Fluent:Notify({
            Title = "Min Gaming",
            Content = "Đã tải cấu hình đã lưu!",
            Duration = 5
        })
    else
        Fluent:Notify({
